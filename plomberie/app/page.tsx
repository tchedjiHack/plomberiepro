...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  // 🔧 1. Formater le téléphone au format international
const formaterTelephoneInternational = (telephone: string): string => {
  // Supprimer tous les espaces, points, tirets
  let nettoye = telephone.replace(/[\s\.\-]/g, '');
  
  // INDICATIF BÉNIN (par défaut)
  const INDICATIF_BENIN = '229';
  
  // Si ça commence par 0 (format local type "90 07 36 83")
  if (nettoye.startsWith('0')) {
    nettoye = '+' + INDICATIF_BENIN + nettoye.substring(1);
  }
  // Si c'est un numéro béninois sans 0 (ex: 90073683)
  else if (nettoye.startsWith('9') && nettoye.length === 8) {
    nettoye = '+' + INDICATIF_BENIN + nettoye;
  }
  // Si ça commence par 6 et fait 8 chiffres (autre format béninois)
  else if (nettoye.startsWith('6') && nettoye.length === 8) {
    nettoye = '+' + INDICATIF_BENIN + nettoye;
  }
  // Si le numéro est déjà au format international avec +, on garde
  else if (!nettoye.startsWith('+')) {
    // Si ça commence par l'indicatif sans le +
    if (nettoye.startsWith(INDICATIF_BENIN)) {
      nettoye = '+' + nettoye;
    }
  }
  ...
  const telephoneInternational = formaterTelephoneInternational(formData.telephone);

  try {
    // ===== PARTIE 1 : Envoi à Supabase (ton code existant) =====
    const insertData: any = {
      nom: formData.nom,
      telephone: formData.telephone,  // On garde le téléphone original en base
      telephone_international: telephoneInternational, // Optionnel : stocker aussi le format international
    ...
    // ===== PARTIE 2 : Envoi à n8n (NOUVEAU) =====
    // 🔴 REMPLACE CETTE URL PAR CELLE DE TON WEBHOOK N8N 🔴
    const webhookUrl = 'https://tchedji.app.n8n.cloud/webhook/0526ea50-fd78-4eb0-af66-ac960f0a9b58';
    
    const payload = {
      first_name: formData.nom,
      email: formData.email || user?.email,
      phone: telephoneInternational,  // ← format international
    ...
    // Envoi asynchrone vers n8n (on ne bloque pas si erreur)
    fetch(webhookUrl, {
      method: 'POST',
      headers: {
      ...
      // On log l'erreur mais on n'affiche rien à l'utilisateur
      console.error('Erreur envoi n8n:', err);
    });

    // ===== PARTIE 3 : Message de succès =====
    setSubmitStatus({
      type: 'success',
      message: user 
        ? '✅ Message envoyé ! Un email et un WhatsApp vont vous être envoyés automatiquement. Suivez le traitement dans votre espace client.'
        : '✅ Message envoyé ! Vous allez recevoir une réponse par email et WhatsApp. Créez un compte pour suivre vos demandes.'
    });
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <div key={index} className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-gray-100 dark:border-gray-700 flex flex-col items-center text-center">
                ...
                {/* <Link href={service.link || "#"} className="mt-6 w-[60%] bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white py-2.5 rounded-xl font-semibold transition-all duration-300 text-center block">
            Découvrir
          </Link> */}
              </div>
            ))}
...
const services = [
  {
    ...
    title: "Dépannage",
    description: "Intervention pour fuites, canalisations bouchées et pannes de chauffe-eau.",
    badge: "24h/24",
  ...
  },
  {
    ...
    title: "Entretien",
    description: "Maintenance préventive de vos installations pour éviter les pannes.",
